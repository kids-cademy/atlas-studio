...
public final class Files extends js.util.Files
{
  ...
  public static MediaSRC mediaSrc(RepositoryObject collectionItem, String mediaFile, String annotation) {
    ...
    return mediaSrc(collectionItem.getRepositoryName(), collectionItem.getName(), Strings.concat(basepath, '_', annotation, '.', extension));
  }
  ...
  /**
   * Get media file path with given base name and extension. Returned file is not tested for existence.
   * 
   * @param collectionItem object owning requested media file,
   * @param basename base name for media file,
  ...
  public static void removeFilesHierarchy(File[] dirs) throws IOException {
    for(File dir : dirs) {
      removeFilesHierarchy(dir).delete();
    }
  }
}

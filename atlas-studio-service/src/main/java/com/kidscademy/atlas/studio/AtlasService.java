...
public interface AtlasService
{
  ...
  /**
   * 
   * @param search optional name search, substring accepted, null if not used,
   ...
   * @param object
   * @param picture
   * @throws NullPointerException if picture instance is null.
   * @throws IOException
   */
  ...
  /**
   * Upload audio sample. Form should have <code>dtype</code> and <code>name</code> values - see {@link AtlasItem}, that are used to create uploaded file path.
   * Also form should have <code>file</code> stream.
   * <p>
   * This method creates object audio sample file, overwriting any existing.
   * 
   * @param form web interface form.
   * @return audio sample info with status after upload.
   * @throws IOException if upload fail for whatever reason.
   */
  AudioSampleInfo uploadAudioSample(Form form) throws IOException;

  /**
   * Normalize object audio sample peak level to 0dB. If file level is already normalized this service does nothing and returns current info state.
   * 
   * @param object object owning audio sample.
   * @return audio sample info with status after processing.
   * @throws IOException if processing fail.
   */
  AudioSampleInfo normalizeAudioSample(AtlasItem object) throws IOException;

  /**
   * Convert object audio sample to mono, if currently is stereo. Before mixing both channels, level is reduced to half in order to avoid peak level trimming.
   * <p>
  ...
  /**
   * Trim silence from both start and end of object audio sample. Inside silence is not processed.
   * <p>
   * Silence trim is performed till found at least half seconds of signal. This allows for eliminating bursts of noises.
   * <p>
   * Implementation note: dues to implementation limitations files larger than {@link CT#MAX_TRIM_FILE_SIZE} are not guaranteed to be processed correctly and is
   * possible to inadvertently remove inside silence.
   * 
   * @param object object owning audio sample.
   * @return audio sample info with status after processing.
   * @throws IOException if processing fail.
  ...
  /**
   * Extract 30 seconds sample from object audio file. Sample is extracted starting from given timestamp, expressed in seconds with decimal.
   * 
   * @param object object owning audio sample.
   * @param start sample start timestamp, in seconds with decimals.
   * @return audio sample info with status after processing.
   * @throws IOException if processing fail.
   */
  ...
  /**
   * Add triangular fade-in of hard coded duration at the beginning of object audio sample. Current hard coded duration is 2.5 seconds.
   * 
   * @param object object owning audio sample.
   * @return audio sample info with status after processing.
   * @throws IOException if processing fail.
  ...
  /**
   * Add triangular fade-out of hard coded duration at the end of object audio sample. Current hard coded duration is 2.5 seconds.
   * 
   * @param object object owning audio sample.
   * @return audio sample info with status after processing.
   * @throws IOException if processing fail.
  ...
  /**
   * Generate waveform image for object audio sample. Generated image file is stored on object media directory - the same directory where audio sample file
   * resides.
   * 
   * @param object object owning audio sample.
  ...
  /**
   * Remove object audio sample from media repository and update database record. If there are not commited audio transforms they are also removed.
   * 
   * @param object object owning audio sample.
   * @throws IOException if processing fail.
   */
  ...
  ExportObject getReaderObject(int objectId, String language);

...
{
  /**
   * Return an excerpt from given paragraph. Select start substring till first punctuation character, exclusive.
   * 
   * @param paragraph
   * @return
   */
  ...
  private static Pattern BASE_DOMAIN_PATTERN = Pattern.compile("^(?:http|https|ftp|file):\\/\\/(?:[^.]+\\.)*([^.]+\\.[^:/]+)\\/.*$");
  ...
  /**
   * Convert HTML formatted into plain text. This method parses source text and detect HTML tags. Current implementation handle only &lt;p&gt; tags replacing
   * them with line break.
   * <p>
   * <b>Implementation note</b>: experimental implementation. This implementation is work in progress and is marked as deprecated to warn developer about logic
   * evolution. Final parser should handle &lt;br&gt; to line break, &lt;p&gt; to double line breaks, &lt;q&gt; to simple quotation mark, &lt;ul&gt; to new
   * lines beginning with tab and dash, &lt;ol&gt; to new lines beginning with tab and ordinal and all &lt;h&gt; tags to triple line breaks.
   * 
  ...
  public static String ellipsis(String text, int maximLength) {
    if(text.length() < maximLength) {
      return text;
    }
    return text.substring(0, maximLength - ELLIPSIS.length()) + ELLIPSIS;
  }
}
